...
- Parameter $$ \theta $$ 에 대한 $$ 1-\alpha $$ **신뢰구간(confidence interval)** 은 아래 식 (2) 를 만족하는 구간 $$ C_{n} = (a,b) $$ 으로 정의된다. 여기서 $$ a = a(X_{1},\ldots, X_{n}) $$, $$ b = b(X_{1},\ldots, X_{n}) $$ 은 데이터에 대한 함수이다. 다시 말해 $$ (a,b) $$ 는 $$ 1-\alpha $$ 확률로 $$ \theta $$ 를 잡는다(trap) 는 의미이다 :
...
- `Warning` 신뢰구간 또는 신뢰집합 $$ C_{n} $$ 은 랜덤이고 $$ \theta $$ 는 고정되어 있는 것이다. 이 점에서 많이 혼동하는데, $$ \theta $$ 는 고정된 값이므로 확률변수가 아니기 때문에 신뢰구간은 $$ \theta $$ 에 관한 probability statement 가 아니다. 다시 말해 $$ \theta $$ 가 $$ 1-\alpha $$ 확률로 신뢰구간에 속하는게 아니라, 우리가 산출한 신뢰구간이 $$ 1-\alpha $$ 확률로 $$ \theta $$ 를 잡는 것이다 
...
- "신뢰구간은 $$ \theta $$ 에 관한 probability statement 가 아니다" 라는 말이 혼동스러운 사람은 아래 Berger $$ \& $$ Wolpert (1984)의 예제를 읽어보자
 
---

`Example` parameter $$ \theta $$ 를 고정하고 $$ X_{1}, X_{2} $$ 를 독립이고 $$ \mathbb{P}(X_{i} = 1) = \mathbb{P}(X_{i} = -1) = 1/2 $$ 인 분포를 가진다고 하자. 우리는 확률변수 $$ Y_{i} = X_{i} + \theta $$ 만 관찰할 수 있다고 가정한다. 이 때 집합 $$ C $$ 를 다음과 같이 정의한다 :

$$
C = \begin{cases} \{Y_{1} - 1\} & : Y_{1} = Y_{2} \\ \{(Y_{1}+Y_{2})/2\}& : Y_{1}\neq Y_{2} \end{cases}
$$

집합 $$ C $$ 는 $$ \theta $$ 가 얼마이든 $$ X_{1} = X_{2} = -1 $$ 만 아니면 $$ \theta \in C $$ 를 만족한다. 그러므로 $$ \mathbb{P}_{\theta}(\theta \in C) = 3/4 $$ 이기 때문에 집합 $$ C $$ 는 75% 신뢰구간이다. 이제 우리에게 $$ Y_{1} = 15, Y_{2} = 17 $$ 이라는 실험결과가 주어졌다고 하자. 이 때 우리의 75% 신뢰구간 $$ C $$ 는 $$ \{ 16 \} $$ 이 된다. 하지만 우리는 이미 $$ Y_{1} \neq Y_{2} $$ 이면 $$ (Y_{1}+Y_{2})/2 = \theta $$ 가 된다는 걸 알고 있으므로 $$ \theta = 16 $$ 으로 확신할 수 있다. 다시 말해 $$ \mathbb{P}(\theta \in C \vert Y_{1}, Y_{2}) = 1 $$ 이다. 분명히 집합 $$ \{ 16 \} $$ 은 75% 신뢰구간이지만, 이는 데이터 $$ X_{1}, X_{2} $$ 에 대한 확률명제이지, $$ \theta $$ 에 대한 확률명제는 아니다.
