...
- 데이터 $$ x $$ 가 분포 $$ x \sim p_{\text{true}}(x) $$ 를 따른다고 하자. 우리는 $$ p_{\text{\true}}(x) $$ 를 알 수 없지만 
- Auto-Encoder (이하 **AE**) 는 
- VAE 는 
...
- 문제는 (항상) 두번째다. 이번에는 $$ \nabla_{\phi} $$ 가 아예 $$ \phi $$ 를 미분하는 연산자이므로 $$ q_{\phi} $$ 도 같이 미분해줘야 한다. 그러므로 기대값에 미분연산을 집어넣으면 큰일난다 :angry: ::sweat::
