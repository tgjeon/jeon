...
- 위에서 소개한 Bertrand paradox 에서 표본공간 \\(\Omega\\) 는 가능한 직선들의 집합으로 공통이지만 \\(\sigma\\)-algebra 는 서로 다르다
	- 답안 1은 **중점의 좌표 \\( (x,y) \\) 가 서로 같은 직선들의 모임** 으로 \\(\sigma\\)-algebra 를 구성한다
	- 답안 2는 **직선과 원의 중심 사이의 거리 \\( 0 \leq d \leq 2 \\) 가 서로 같은 직선들의 모임** 으로 \\(\sigma\\)-algebra 를 구성한다
	- 답안 3은 **보조선과 직선의 각도 \\( 0\leq \theta \leq \pi \\) 가 서로 같은 직선들의 모임** 으로 \\(\sigma\\)-algebra 를 구성한다
