...
- 위 기호를 \\( \mathbb{P}(X \in A) = \mathbb{P}_{X}(A) \\) 로도 표기한다. \\( \mathbb{P}_{X}(\cdot) \\) 는 \\( \mathbb{R} \\) 상에서 [Kolmogorov Probability Axiom](https://sungbinlim.github.io/sl/docs/mpt/2) 을 만족하는 확률측도(Probability measure)가 된다. 앞으로 우리는 \\( \mathbb{P}_{X}(\cdot) \\) 를 \\( X \\) 의 **확률분포(Probability distribution)** 라 부른다.
