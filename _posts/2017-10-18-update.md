...
두 risk 의 차이를 generalization gap 이라 하는데, 오버피팅(overfitting)을 논할 때 사용되는 generalization error 는 식 (1) 의 추정량(estimator)이 된다 :
...
그런데 ERM 으로 원래 목표인 $$ R[f_{\mathcal{A}(S_{m})}] $$ 최소화가 반드시 가능할까? 

$$
\text{minimize }\hat{R}_{m}[f_{\mathcal{A}(S_{m})}]\quad\Longrightarrow\quad  \text{minimize }R[f_{\mathcal{A}(S_{m})}]
$$

사실 오버피팅은 위 명제가 성립하지 않는 경우에 일어난다. 다시 말해 Generalization 이론의 목적은 식 (1) 의 generalization gap 을 분석함으로써 이를 규명하는게 목표다. 
