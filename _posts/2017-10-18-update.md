...
- 머신러닝 이론에서 학습의 목표는 expected risk $$ R[f_{\mathcal{A}(S_{m})}] $$ 를 최소화하는 것이다
- 그러나 expected risk 는 직접 계산하는 건 불가능하다 (non-computable)
- 대신 empirical risk 인 $$ \hat{R}_{m}[f_{\mathcal{A}(S_{m})}] $$ 을 최소화한다. 이를 empirical risk minimization (이하 **ERM**) 이라 부른다
- 두 risk 의 차이를 generalization gap 이라 부른다 :

$$
\text{generalization gap}:= R[f_{\mathcal{A}(S_{m})}] - \hat{R}_{m}[f_{\mathcal{A}(S_{m})}]\quad\cdots\quad(1)
$$

- 우리가 말하는 generalization error 는 test error 와 train error 의 차이로 정의하는데, 이는 식 (1) 의 추정량(estimator)이다

- 그런데 ERM 으로 $$ R[f_{\mathcal{A}(S_{m})}] $$ 최소화가 가능할까? 이게 가능하려면 필요조건으로 오버피팅(overfitting)이 일어나지 않아야 한다.

- Generalization 이론의 목적은 식 (1) 의 generalization gap 을 분석함으로써 이를 규명하는게 목표다. 

- 문제는 식 (1) 의 분석이 쉬운 일은 아니다. 
	- 왜냐하면 empirical risk 에서 학습된 모델과 $$ \hat{R}_{m} $$ 둘 다 학습데이터 $$ S_{m} $$ 에 의존하는 통계량이기 때문이다 :

$$
\hat{R}_{m}[f_{\mathcal{A}(S_{m})}] = \frac{1}{m}\sum_{i=1}^{m}\ell(f_{\mathcal{A}(S_{m})}(x_{i}),y_{i}),\quad (x_{i},y_{i})\sim P(x,y)
$$

- 이 부분을 돌파하기 위해 Generalization 이론에서는 여러가지 분석 방법론을 개발해왔다
