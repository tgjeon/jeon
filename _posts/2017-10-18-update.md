...
머신러닝 이론에서 학습의 목표는 expected risk $$ R[f_{\mathcal{A}(S_{m})}] $$ 를 최소화하는 것이다. 그러나 expected risk 를 직접 계산하는 건 불가능하기 때문에 대신 empirical risk 인 $$ \hat{R}_{m}[f_{\mathcal{A}(S_{m})}] $$ 을 최소화한다. 이를 empirical risk minimization (이하 **ERM**) 이라 부르고, 두 risk 의 차이를 generalization gap 이라 부른다 :
...
그런데 ERM 으로 $$ R[f_{\mathcal{A}(S_{m})}] $$ 최소화가 가능할까? 이게 가능하려면 필요조건으로 오버피팅(overfitting)이 일어나지 않아야 한다. Generalization 이론의 목적은 식 (1) 의 generalization gap 을 분석함으로써 이를 규명하는게 목표다. 
