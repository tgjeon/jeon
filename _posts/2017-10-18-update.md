...
- 실제로는 empirical risk 인 $$ \hat{R}_{m}[f_{\mathcal{A}(S_{m})}] $$ 을 최소화한다. 이를 empirical risk minimization (이하 **ERM**) 이라 부른다
...
- 유사하게 Generalization error 를 test error 와 train error 의 차이로 정의하는데, 이는 식 (1) 의 추정량(estimator)이다
- 실제 데이터에서는 $$ R[f_{\mathcal{A}(S_{m})}] $$ 을 관찰할 수 없기 때문이다.

- 그런데 ERM 으로 $$ R[f_{\mathcal{A}(S_{m})}] $$ 최소화가 가능할까?
- 이게 가능하려면 필요조건으로 오버피팅(overfitting)이 일어나지 않아야 한다.
